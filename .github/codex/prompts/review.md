...
**レビューを開始する前に、以下のファイルを必ず読み込んでください：**

1. **`changed_files.txt`** - PRで変更されたファイル一覧
2. **`pr_diff.patch`** - PRの差分内容（unified diff形式）

これらのファイルはワークスペースのルートディレクトリにあります。
まずこれらを読み込んで、PRの変更内容を把握してください。
...
**重要**: これらのファイルが読み込めない場合や空の場合は、レビュー対象の変更がないと判断してください。
