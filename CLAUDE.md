...
1. 作業完了後、git commit してプルリクエストを作成
2. プルリクエスト後は必ず Codex Review + CI の完了を待つ
3. REQUEST_CHANGES があれば全て修正して再push。変更外の場合は当該箇所を確認して修正。レビューに「対応する必要がない」場合は、その旨をIssueのコメントに記載して5へ
4. 修正後したら再度pushを行い 2 へ
5. プルリクエストをMergeする
```
