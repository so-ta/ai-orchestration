...
これにより、CIの失敗を事前に防ぎ、レビューサイクルを短縮できる。
...
```
Push前に必ず確認:
1. [ ] Backend変更 → go test ./... がパス
2. [ ] Frontend変更 → npm run check がパス
3. [ ] コンフリクトマーカーが残っていないこと
4. [ ] 機密情報（APIキー、パスワード等）が含まれていないこと
```
...
| 失敗内容 | 対応 |
|---------|------|
| テスト失敗 | テストを修正してから再度コミット・push |
| 型エラー | 型エラーを修正してから再度コミット・push |
| lint エラー | lintエラーを修正してから再度コミット・push |

**重要**: ローカルCIが通らない状態でpushすることは禁止。
